    ...
    begin
        if rising_edge(clk) then
            if resetn = '0' then
                ...
                if pressed(1) = '1' then --switched to respond on btn(1) instead of btn(0) based on tb CHANGED CONDITIONAL BACK: falling(1) = '1' and forcedModePrev = '0' then
                    forcedMode <= '1';
                elsif pressed(1) = '1' then --CHANGE CONDITIONAL BACK falling(1) = '1' and forcedModePrev = '1' the
                    forcedMode <= '0'; 
                end if;
 	...
 	datapath_inst: acquireToHDMI_datapath
 	  port map ( 
 	        ...
 	        triggerTimePixel => std_logic_vector(to_unsigned(105,VIDEO_WIDTH_IN_BITS)),  --hardwire to left side of screen
